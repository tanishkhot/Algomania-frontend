...
export function LoginFormDemo() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try{
      const response = await axios.post("https://algomania-backend.onrender.com/api/login/", formData);
  ...
  return (
    <div className="max-w-md w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-gray-700/30 backdrop-blur-xl border border-gray-100/20">
      ...
      <p className="text-neutral-600 text-sm max-w-sm mt-2 dark:text-neutral-300">
        The Utimate DSA Challenge
      </p>

      <form className="my-8" onSubmit={handleSubmit}>
        <LabelInputContainer className="mb-4">
          <Label htmlFor="email">Email Address</Label>
          <Input id="email" placeholder="projectmayhem@fc.com" value={formData.email}type="email" onChange={handleChange} />
        </LabelInputContainer>
        <LabelInputContainer className="mb-4">
